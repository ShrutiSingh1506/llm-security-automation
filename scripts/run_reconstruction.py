...
Usage: python scripts/run_reconstruction.py
"""
...
def update_dashboard(chains: List[AttackChain]) -> None:
    ...
    ">
        ...
        <p style="color: rgba(255,255,255,0.6); font-size: 0.9rem; margin-bottom: 24px;">
            Kill chain mapping across {len(chains)} log source(s) · MITRE ATT&CK aligned
        </p>
    ...
    DASHBOARD_RECONSTRUCTION_FILE.write_text(html)
    logger.info("Dashboard saved to %s", DASHBOARD_RECONSTRUCTION_FILE)
    print(f"\n  Dashboard created: {DASHBOARD_RECONSTRUCTION_FILE}")
    print(f"  Open with: open {DASHBOARD_RECONSTRUCTION_FILE}")
...
<div class="header">
  ...
  <div class="subtitle">LLM Security Platform &mdash; {generated}</div>
  <div class="stats">
...
{chain_html}
<div class="footer">
  Attack Chain Reconstruction — LLM Security Platform<br>
...
def main() -> None:
    ...
    print(f"  {DASHBOARD_RECONSTRUCTION_FILE}  — dashboard with attack chains")
    print(f"\n  Open: open {DASHBOARD_RECONSTRUCTION_FILE}")
