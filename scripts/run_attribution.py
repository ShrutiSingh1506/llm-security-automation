"""
Entry point: run threat actor attribution on reconstructed attack chains.
...
from config import ATTACK_CHAINS_FILE, OUTPUT_DIR, DASHBOARD_RECONSTRUCTION_FILE, DASHBOARD_ATTRIBUTION_FILE
...
def main() -> None:
    ...
    if not chains:
        ...
        print(f"  Run scripts/run_reconstruction.py first to generate {ATTACK_CHAINS_FILE}")
        sys.exit(0)
    ...
    if not attributions:
        print("\n  No attributions produced — check your OpenAI API key.")
        sys.exit(0)
    ...
    inject_into_dashboard(attributions, DASHBOARD_RECONSTRUCTION_FILE, DASHBOARD_ATTRIBUTION_FILE)

    print("\nDone.")
    print(f"  {ATTRIBUTION_FILE}  — structured attribution data")
    print(f"  {DASHBOARD_ATTRIBUTION_FILE}  — dashboard with attribution")
    print(f"\n  Open: open {DASHBOARD_ATTRIBUTION_FILE}")
